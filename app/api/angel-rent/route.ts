...
// CON EDICIÓN HABILITADA - Proxy residencial siempre activo
// ═══════════════════════════════════════════════════════════════════════════
...
async function handle(req: NextRequest, method: string): Promise<Response> {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════════
    // Se eliminó el bloqueo de la página /edit
    // Ahora funciona con proxy residencial para todo
...
@media (max-width: 768px){#ar-bar{height:44px}.ars{padding:0 10px;gap:4px}.ars:first-child{padding-left:8px}.arl{font-size:8px;letter-spacing:.7px}.arv{font-size:11px}#ar-logo-icon{width:24px;height:24px;font-size:13px;border-radius:7px}@media (max-width: 480px){.ars-hide-mobile{display:none!important}}}
...
<div id="ar-sbox">
  ...
  // El JavaScript continúa en la siguiente parte del archivo...
  const script = `<script>
(function(){
"use strict";
...
function toggleRobot(){var s=gst();var ring=document.getElementById("ar-pulse-ring");if(s.on){s.on=false;s.nextAt=0;sst(s);if(TICK){clearInterval(TICK);TICK=null;}addLog("in","Robot OFF");saveRobotState(false,false);if(ring)ring.style.display="none";}else{s.on=true;s.paused=false;s.cnt=0;sst(s);addLog("ok","Robot ON - bumps 16-20 min");saveRobotState(true,false);schedNext();startTick();doBump();if(ring)ring.style.display="block";}updateUI();}
...
for(var i=0;i<allLinks.length;i++){var el=allLinks[i];var text=(el.innerText||el.textContent||"").trim().toUpperCase();var href=(el.getAttribute("href")||"").toLowerCase();
if(text.indexOf("REMOVE POST")!==-1||text.indexOf("DELETE POST")!==-1||text.indexOf("DELETE ACCOUNT")!==-1||text.indexOf("REMOVE ACCOUNT")!==-1){el.style.opacity="0.5";el.style.cursor="not-allowed";el.style.filter="grayscale(1)";el.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();alert("⚠️ Acción deshabilitada. Contacta soporte.");},true);}
if((href.indexOf("/delete")!==-1||href.indexOf("/remove")!==-1)&&href.indexOf("/bump")===-1&&href.indexOf("/repost")===-1){el.style.opacity="0.5";el.style.cursor="not-allowed";el.style.filter="grayscale(1)";el.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();alert("⚠️ Acción deshabilitada. Contacta soporte.");},true);}}
},1000);
setInterval(function(){var dangerousButtons=document.querySelectorAll("a,button");for(var i=0;i<dangerousButtons.length;i++){var el=dangerousButtons[i];var text=(el.innerText||el.textContent||"").trim().toUpperCase();var href=(el.getAttribute("href")||"").toLowerCase();
if((text.indexOf("REMOVE POST")!==-1||text.indexOf("DELETE")!==-1||href.indexOf("/delete")!==-1||href.indexOf("/remove")!==-1)&&href.indexOf("/bump")===-1&&href.indexOf("/repost")===-1){if(el.style.opacity!=="0.5"){el.style.opacity="0.5";el.style.cursor="not-allowed";el.style.filter="grayscale(1)";el.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();alert("⚠️ Acción deshabilitada. Contacta soporte.");},true);}}}},3000);
