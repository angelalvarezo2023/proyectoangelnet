...
async function handle(req: NextRequest, method: string): Promise<Response> {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════════
    // ESTRATEGIA: Usar IP REAL del cliente para EDITAR
    // ═══════════════════════════════════════════════════════════════════
    const isEditPage = decoded.includes("/users/posts/edit") || 
                       decoded.includes("/users/posts/create");
    
    // Si está editando, NO usar proxy (usar IP real del cliente)
    const shouldUseProxy = !isEditPage;
    
    if (isEditPage) {
      console.log("[Angel Rent] EDIT MODE: Using client's real IP (bypassing proxy)");
    }
    ...
    const agent = (PH && PT && shouldUseProxy) 
    ...
    // Log para debugging
    if (!shouldUseProxy) {
      console.log("[Angel Rent] Request made with REAL IP (no proxy)");
    } else {
      console.log("[Angel Rent] Request made through PROXY");
    }
...
<div id="ar-mbox">
  ...
  const uiHtml = `
${modalHtml}
${isEditPage ? '<div id="ar-real-ip-banner" style="position:fixed;top:48px;left:0;right:0;z-index:2147483646;background:linear-gradient(90deg,#10b981,#059669);padding:6px 14px;text-align:center;font-family:-apple-system,sans-serif;font-size:11px;font-weight:800;color:#fff;box-shadow:0 2px 12px rgba(16,185,129,.4);animation:ar-promo-in .4s ease"><span style="margin-right:6px">🔓</span>Modo Edición: Usando tu IP real (sin proxy) para evitar bloqueos<span style="margin-left:6px">✨</span></div>' : ''}
<div id="ar-bar">
  <div class="ars">
...
// ═══════════════════════════════════════════════════════════════════════
// DETECTAR CUANDO SALE DE LA PÁGINA DE EDICIÓN PARA VOLVER AL PROXY
// ═══════════════════════════════════════════════════════════════════════
if(C.indexOf("/users/posts/edit")!==-1 || C.indexOf("/users/posts/create")!==-1){
  console.log("[Angel Rent] ⚠️ EDIT MODE ACTIVE - Using real IP");
  console.log("[Angel Rent] After completing edit, will return to proxy mode");
  
  // Detectar cuando se completa la edición
  var checkInterval = setInterval(function(){
    var url = window.location.href;
    // Si ya no está en edit/create, significa que terminó
    if(url.indexOf("/users/posts/edit") === -1 && url.indexOf("/users/posts/create") === -1){
      console.log("[Angel Rent] ✅ Edit completed, returning to PROXY mode");
      clearInterval(checkInterval);
      
      // Mostrar banner de éxito
      var banner = document.getElementById("ar-real-ip-banner");
      if(banner){
        banner.style.background = "linear-gradient(90deg,#3b82f6,#1d4ed8)";
        banner.innerHTML = '<span style="margin-right:6px">✅</span>Edición completada - Volviendo al modo proxy protegido<span style="margin-left:6px">🔒</span>';
        setTimeout(function(){
